...
def test_section_1_configuration(results: TestResult):
    ...
    if not missing_dirs:
        results.add_test(
            ...
            True,
            "rag/, data_regle/, storage/ présents"
        )
    else:
        results.add_test(
            "1.2 - Structure des dossiers",
...
def test_section_2_rag(results: TestResult):
    ...
    try:
        ...
        results.add_test(
            "2.1 - Imports module RAG",
            True,
            "engine, models, guardrails importés"
        )
    except ImportError as e:
        results.add_test(
            "2.1 - Imports module RAG",
    ...
    if engine.protocol_index is not None:
        ...
        if nb_vectors > 0:
            results.add_test(
                "2.5 - Index FAISS",
                True,
                f"{nb_vectors} vecteurs dans l'index"
            )
        else:
            results.add_test(
                "2.5 - Index FAISS",
                False,
                "Index chargé mais vide (0 vecteurs)"
            )
    else:
        results.add_test(
            "2.5 - Index FAISS",
    ...
    try:
        ...
        if rag_response.is_safe:
            ...
            print(f"    ℹ️  Note : En contexte hospitalier, mieux vaut un faux positif qu'une injection réussie")
    except Exception as e:
        results.add_test(
            "2.8 - Requête RAG safe",
            False,
...
def test_section_3_agent(results: TestResult):
    ...
    try:
        ...
        if "1455" in str(e):
            ...
            print("    💡 Solution : Modifiez rag/guardrails.py pour utiliser @property (voir documentation)")
...
def test_section_4_mcp(results: TestResult):
    ...
    try:
        ...
        response = requests.post(
            f"{MCP_URL}/tools/ajouter_patient",
            json=patient_data,
            timeout=3
        )
...
def main():
    ...
    if results.failed > 0:
        ...
        print("   2. Si serveur MCP inaccessible : Lancez 'python server.py' dans un autre terminal")
