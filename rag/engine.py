...
class HospitalRAGEngine:
    ...
    def __init__(
        ...
        logger.info(
            f"Hospital RAG Engine initialisé en mode '{mode}' (Threshold: {self.MIN_CONFIDENCE_THRESHOLD})"
        )
    ...
    def query(self, user_query: str, wait_time: int = 0) -> RAGResponse:
        ...
        if similarity_score < self.MIN_CONFIDENCE_THRESHOLD:
            logger.warning(
                f"Rejet par seuil de confiance : {similarity_score:.4f} < {self.MIN_CONFIDENCE_THRESHOLD}"
            )
            return self._build_error_response(
    ...
    def _verify_input_safety(self, query: str) -> GuardrailResult:
        try:
            is_safe, threat_score, embedding, reason = self.guardrail.verify_input(
                query
            )
            if not is_safe:
    ...
    def _search_protocol(
        self, query_embedding: npt.NDArray[np.float32]
        ...
        logger.debug(
            f"FAISS Search | Index: {best_idx} | Score: {similarity_score:.4f}"
        )
