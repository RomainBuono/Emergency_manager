...
class InjectionDetector:
    ...
    INJECTION_PATTERNS: Final[tuple[Pattern, ...]] = (
        ...
        re.compile(
            r"ignore\s+(previous|preceding|instructions?|prompts?)", re.IGNORECASE
        ),
        re.compile(
            r"(oublie|forget|discard)\s+(ton rôle|your role|instructions?)",
            re.IGNORECASE,
        ),
        re.compile(
            r"(tu es|you are)\s+(maintenant|now)\s+(?!un patient|le médecin)",
            re.IGNORECASE,
        ),
        re.compile(r"mode\s+(développeur|developer|admin|debug|test)", re.IGNORECASE),
        re.compile(
            r"(désactive|disable|override)\s+(sécurité|security|validation|protocole)",
            re.IGNORECASE,
        ),
        ...
        re.compile(
            r"\b(SELECT|INSERT|UPDATE|DELETE|DROP|CREATE|ALTER)\s+", re.IGNORECASE
        ),
...
class MedicalLogicValidator:
    ...
    @classmethod
    def validate(
        cls, protocol: MedicalProtocol, rules: list[HospitalRule], wait_time: int = 0
    ) -> tuple[bool, str]:
        """
        ...
        if protocol.gravite == "JAUNE" and wait_time > cls.MAX_WAIT_JAUNE:
            if not any(
                "réévaluation" in getattr(r, "titre", "").lower() for r in rules
            ):
...
class RAGGuardrail:
    ...
    def __init__(
        self, config: Optional[GuardrailConfig] = None, use_ml: bool = True
    ) -> None:
        """
        ...
        if use_ml:
            logger.info(
                "Mode chatbot : Guardrails ML activés (sécurisé mais plus lent)"
            )
        else:
    ...
    def verify_relevance(self, query: str, score: float) -> tuple[bool, str]:
        ...
        reason = (
            "Sufficient"
            if is_relevant
            else f"Score {score:.3f} < {self.config.min_relevance}"
        )

        return is_relevant, reason

    def verify_logic(
        self, protocol: MedicalProtocol, rules: list[HospitalRule], wait_time: int = 0
    ) -> tuple[bool, str]:
        """
