...
class GuardrailTrainer:
    ...
    def _normalize_dataset(
        self, dataset: Dataset, text_col: str, label_val: int
    ) -> Dataset:
        """
    ...
    def _get_medical_safe_queries(self) -> Dataset:
        ...
        return Dataset.from_dict(
            {"text": queries * 100, "label": [0] * (len(queries) * 100)}
        )
    ...
    def prepare_data(self) -> Dataset:
        ...
        ds_wiki = ds_wiki.filter(
            lambda x: len(x["text"]) > 40
        )  # Filtre les titres courts
