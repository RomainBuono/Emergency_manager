...
def normalize_dataset(
    dataset, text_col: str, label_col: str, target_label_map: Dict[int, int]
):
...
def prepare_datasets_robust():
    ...
    full_dataset = concatenate_datasets(
        [ds_deepset, ds_multijail, ds_safe_med, ds_wiki]
    )
...
def main():
    ...
    results.append(
        evaluate_and_report(best_hgb, X_test, y_test, "HistGradientBoosting", time_hgb)
    )
    ...
    results.append(
        evaluate_and_report(best_rf, X_test, y_test, "RandomForest", time_rf)
    )
    ...
    print(
        res_df[["F1", "FPR", "FNR", "Inference_Speed"]]
        .style.format(
            {
                "F1": "{:.2%}",
                "FPR": "{:.2%}",
                "FNR": "{:.2%}",
                "Inference_Speed": "{:.0f} q/s",
            }
        )
        .to_string()
    )
