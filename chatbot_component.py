...
def render_chatbot_premium(
    ...
    st.markdown(
        '<div style="font-size: 0.85rem; color: var(--text-secondary); margin-bottom: 12px;">⚡ Actions rapides :</div>',
        unsafe_allow_html=True,
    )
    ...
    with col2:
        if st.button(
            "👥 Liste patients", use_container_width=True, key="quick_patients"
        ):
    ...
    with col4:
        if st.button(
            "🤖 Dernière décision", use_container_width=True, key="quick_decision"
        ):
